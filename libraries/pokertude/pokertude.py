...
class Analyzer:
    ...
    def analyze(self, verbose=False) -> dict:
        wins = 0
        ties = 0
        ...
        if verbose:
            print(
                "Wins: {0:.2f}% (Expected winnings: {1:.2f}%)".format(100 * win_ratio, 100 * exp_winnings)
            )
            print("Ties: {0:.2f}%".format(ties / total_rounds * 100))
            print("Sources of loss:")
            for rank, count in lossers.most_common():
                print("{0}: {1:.2f}%".format(RANK_TO_STRING[rank], count / total_opponent_hands * 100))
            print()

        ways_to_lose = {}
        for rank, count in lossers.most_common():
            ways_to_lose[RANK_TO_STRING[rank]] = count / total_opponent_hands
            ways_to_lose[RANK_TO_STRING[rank] + '%'] = '{0:.2f}%'.format(count / total_opponent_hands * 100)

        return {
            'win': win_ratio,
            'win%': '{0:.2f}%'.format(100 * win_ratio),
            'win_expected': exp_winnings,
            'win_expected%': '{0:.2f}%'.format(100 * exp_winnings),
            'tie': ties / total_rounds,
            'tie%': '{0:.2f}%'.format(ties / total_rounds * 100),
            'loss': 1 - win_ratio,
            'loss%': '{0:.2f}%'.format(100 * (1 - win_ratio)),
            'ways_to_lose': ways_to_lose
        }
